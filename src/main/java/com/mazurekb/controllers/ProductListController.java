...
public class ProductListController {
	...
	private void addButton() {
		...
		Query query = session.createQuery("Select 1 from Products WHERE productname = '" + productNameField.getText()
				+ "' AND productType = '" + productType2 + "'");
		if (query.uniqueResult() != null) {
			info.setText("That product name is already used!");
			HibernateUtil.shutdown();
		} else {
			query = session.createNativeQuery(
					"INSERT INTO Products (productname, kcal, fat, protein, carbohydrates, producttype) VALUES('"
							+ productNameField.getText() + "','" + kcal + "','" + fat + "','" + protein + "','" + carbo
							+ "','" + productType2 + "')");
			query.executeUpdate();
	...
	private void updateButton() {
		...
		Query query = session.createNativeQuery("UPDATE Products SET productname = '" + productNameField.getText()
				+ "', kcal = '" + kcal + "',fat = '" + fat + "',carbohydrates = '" + carbo + "',protein = '" + protein
				+ "', producttype = '" + productType2 + "' WHERE  productname = '" + selected_product + "'");
		query.executeUpdate();
	...
	private void choiceBox() {
		...
		productChoiceBox2.getSelectionModel().selectedIndexProperty().addListener(new ChangeListener<Number>() {
			public void changed(ObservableValue ov, Number value, Number new_value) {
				productType2 = products_strings[new_value.intValue()];
			}
		});
	}
